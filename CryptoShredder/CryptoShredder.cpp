...
CryptoShredder::CryptoShredder(QWidget* parent) : QMainWindow(parent) {
    ...
    this->infoIconLabel = new IconLabel("images/infoIcon.png", QPoint(1020, 10), QSize(40, 40), this); //set the info icon in GUI
    this->infoIconLabel->setToolTip("<html><head/><body><p><span style='font-size:10pt;'>General information<br>about CryptoShredder.</span></p></body></html>"); //set toolTip for info icon
    this->optionsIconLabel = new IconLabel("images/cipherIcon.png", QPoint(955, 10), QSize(40, 40), this); //set the options icon in GUI
    this->optionsIconLabel->setToolTip("<html><head/><body><p><span style='font-size:10pt;'>Encrypt/Decrypt files.</span></p></body></html>"); //set toolTip for options icon
    ...
    connect(this->ui.CipherCheckBox, &QCheckBox::checkStateChanged, this, &CryptoShredder::cipherCheckBoxClicked);
    ...
    connect(this->infoIconLabel, &IconLabel::clicked, this, &CryptoShredder::infoLabelClicked);
    connect(this->optionsIconLabel, &IconLabel::clicked, this, &CryptoShredder::optionsLabelClicked);
}
...
CryptoShredder::~CryptoShredder() {
    ...
    delete this->infoIconLabel; //delete the infoIconLabel object
    delete this->optionsIconLabel; //delete the optionsIconLabel object
}
...
void CryptoShredder::updateListView(const QString& fileDictionaryName, const QString& fileName, const QString& tag) {
    ...
    if (this->fileHandler) { //update only if fileHandler object is initialized
        int fileIndex = (int)this->fileDictionary[fileDictionaryName.toStdString()]; //get the index of the file we need to update in fileDictionary
...
void CryptoShredder::checkThreads() {
    ...
    if (this->fileHandler) { //check only if fileHandler object is initialized
        ...
        int incrementAmount = 100 / (int)this->filePathList.size(); //increment the progress bar with an amount that represents the division of num of files
...
void CryptoShredder::optionsLabelClicked() {
    if (this->fileHandler) { //if true there's a process in progress
        ...
        if (!CryptoShredder::isWipe) { //if true we change to wipe mode
            this->optionsIconLabel->setNewImage("images/cipherIcon.png", "Encrypt/Decrypt files."); //set optionsIconLabel
            ...
            this->optionsIconLabel->setNewImage("images/wipeIcon.png", "Wipe files."); //set optionsIconLabel
