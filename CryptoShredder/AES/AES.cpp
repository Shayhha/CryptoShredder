 ...
 * @return • string hex
 */
const string AES::VectorToHex(const vector<unsigned char>& vec) {
 ...
 */
const vector<unsigned char> AES::HexToVector(const string& hex) {
...
const vector<unsigned char> AES::Decrypt_ECB(vector<unsigned char>& text, const vector<unsigned char>& key) {
    ...
    if (padding > 0 && padding <= BlockSize) { //if true we have padding bytes to remove from text
        for (size_t i = text.size() - padding; i < text.size(); i++) //check if last bytes match padding value
